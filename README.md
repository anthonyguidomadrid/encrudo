...
- `CONTENTFUL_GRAPHQL_ENDPOINT`: Contentful GraphQL base endpoint used by codegen (this repo appends `/${CONTENTFUL_SPACE_ID}`), e.g. `https://graphql.contentful.com/content/v1/spaces`
- `CONTENTFUL_ENVIRONMENT`: Optional Contentful environment name (used for sitemap fetching), defaults to `master`
- `SMTP_HOST` / `SMTP_PORT` / `SMTP_SECURE`: SMTP server settings (works with SendGrid SMTP and most providers)
...
- `RECAPTCHA_SECRET_KEY`: Server-side reCAPTCHA secret used by the contact API route
- `NEXT_PUBLIC_GA_ID` / `NEXT_PUBLIC_GTAG_ID`: Google Analytics/Tag Manager keys

Notes:
- Email sending is implemented with `nodemailer` over SMTP (there is no direct SendGrid SDK integration).
- `NEXT_PUBLIC_MAILCHIMP_URL` exists in `.env.example`, but newsletter signup currently uses a Mailchimp URL coming from Contentful layout data (not the env var).
...
- Sitemaps are generated automatically after `yarn build` (via the `postbuild` script).
- `public/sitemap*.xml` and `public/robots.txt` are generated by `next-sitemap` and are gitignored.
- Configure your domain and sitemap options in `next-sitemap.config.js`.

### Static Assets

Static images and SVGs live under `public/assets`. Translations live under `public/locales`.

Favicons are currently injected at runtime from Contentful layout data (see the app/layout data fetching), so there may not be a `public/favicons` folder in the repo.
...
Some heavier, client-only components are loaded using [Next.js dynamic imports](https://nextjs.org/docs/advanced-features/dynamic-import) to improve performance and reduce initial bundle size (e.g. reCAPTCHA, lightbox, Mailchimp subscribe).

Header/Footer are rendered server-side to keep internal navigation crawlable.

### PWA (next-pwa)

This project includes `next-pwa`, but the service worker is not registered by default (`register: false`). If you want offline caching, you’ll need to enable registration and ensure the generated `sw.js` is being emitted and served.
...
- **Sitemap not updating**: Ensure `NEXT_PUBLIC_SITE_URL` is set and run `yarn build` (it runs `next-sitemap` via `postbuild`). Generated sitemap/robots files are gitignored and should not be edited by hand.
- **Static assets not updating**: Clear your browser cache or CDN cache after updating files in `public/assets`.
